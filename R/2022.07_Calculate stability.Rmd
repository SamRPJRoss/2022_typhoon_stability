...
kmean_landuse <- kmeans(Landuse_transform, centers = k, nstart = 50)
PCA_out$group<-kmean_landuse$cluster
PCA_out$group[PCA_out$group %in% '1']<-'Forest'
PCA_out$group[PCA_out$group %in% '2']<-'Developed'
PCA_out$group<-parse_factor(as.character(PCA_out$group))
ggbiplot(PCA_out,alpha = 0.25,varname.size = 4,varname.adjust = 1.2,groups = PCA_out$group,ellipse = T) +
  theme_light() + 
  theme(text = element_text(size = 16), 
...
The above analyses are all for our acoustic index time series. We also have data on individual species detections for a few species of birds. First, let's process the data into a useful format. The data come in as individual detections, so we'll generate an empty time series of dates and then sum the number of detections for each species at each site on each date to get our data for analysis. 

First, generate dataframe: 

```{r}

# remove data from 2017 (error) - run twice to remove 2 2017 dates
#Species_detections<-Species_detections[Species_detections$Date %!in% min(unique(Species_detections$Date)),]
#Species_detections<-Species_detections[Species_detections$Date %!in% min(unique(Species_detections$Date)),]


# worst code I've ever written. Can neaten this up in final version once we know how many species, sites, dates etc we have
Daily_species<-data.frame('Site_ID'=rep(unique(Species_detections$Site_ID),each=length(seq(as.Date(min(unique(Species_detections$Date))),as.Date(max(unique(Species_detections$Date))),"days"))*length(unique(Species_detections$Species_ID))),
                          'Species_ID'=rep(rep(unique(Species_detections$Species_ID),each=length(seq(as.Date(min(unique(Species_detections$Date))),as.Date(max(unique(Species_detections$Date))),"days")), length(unique(Species_detections$Site_ID)))),
                          'Date' = rep(seq(as.Date(min(unique(Species_detections$Date))),as.Date(max(unique(Species_detections$Date))),"days"),length(unique(Species_detections$Site_ID))*length(unique(Species_detections$Species_ID))),
...
Now for each site at each confidence cutoff threshold (0.5,0.75,0.9), we'll count the number of calls of each species per day and store it in the dataframe:
...
Now let's make an output dataframe for the stability results:

```{r}

# Make output dataframe
Detection_stability<-unique(Daily_species[,c(1,2,5)])
Detection_stability<-cbind(Detection_stability,"Baseline"=NA,"Temp_Var"=NA,"Ext_Chng"=NA,"n_pre"=NA,"n_post"=NA,"Mean_post"=NA,"Wilcox_W"=NA,"Chng_hi"=NA,"Chng_lo"=NA,"p_val"=NA,"p_sig"=1,"Lat"=NA,"Long"=NA,"kmeans"=NA)

# add land use variables
for (i in 1:length(Detection_stability$Site_ID)) {
  Detection_stability[i,c(15:17)]<-AI_stability[AI_stability$Site_ID %in% Detection_stability$Site_ID[i],c(9:10,22)][1,]
}

```

## Species detections Baseline

As with the Acoustic indices, we'll calculate the baseline as the mean of the pre-typhoon period at each site for each species (and detection cutoff threshold):
...
for (i in 1:length(Detection_stability$Site_ID)) {
  ...
  # calculate baseline of species detection time series at each site as mean of pre-typhoon period
    Detection_stability$Baseline[i]<-Daily_species$Detect.Num[Daily_species$Site_ID %in% Detection_stability$Site_ID[i] &
...
So now we have our mean values of the pre-disturbance period (that is, our baseline value). 

## Calculate species detection temporal variability

As above, we'll calculate the temporal variability of the pre-disturbance period as the coefficient of variation: 

```{r}

for (i in 1:length(Detection_stability$Site_ID)) {
  
        # calculate temporal variability of species detection time series at each site as sd/mean of pre-typhoon period
    Detection_stability$Temp_Var[i]<-
...
**Convert to 1 - Temporal Variability**

```{r}

for (i in 1:length(unique(Detection_stability$Species_ID))) { 

Detection_stability$Temp_Var[Detection_stability$Species_ID %in% unique(Detection_stability$Species_ID)[i]]<-1-
    (Detection_stability$Temp_Var[Detection_stability$Species_ID %in% unique(Detection_stability$Species_ID)[i]]/
       max(Detection_stability$Temp_Var[Detection_stability$Species_ID %in% unique(Detection_stability$Species_ID)[i]],na.rm = T))
...
NB: Before we calculate the extent of change below, we need to remove all cases in our output dataframe where we don't have data. They're easy to spot because the Baseline is Zero and the Temporal Variability is NaN:

```{r}

# remove cases where there are no detections before the typhoon
Detection_stability<-Detection_stability[Detection_stability$Baseline %!in% 0,]

```

## Calculate species detection extent of change

Our final dimension of stability is extent of change as above. We'll again use a Wilcoxon Signed rank test to make before versus after typhoon comparisons:

```{r}

## calculate extent of change for species detection time series
for (i in 1:length(Detection_stability$Site_ID)) {
  
      # separate the pre- and post-typhoon data
      Data_pre<-data.frame("Detect" = Daily_species$Detect.Num[Daily_species$Site_ID %in% Detection_stability$Site_ID[i] &
                                                                Daily_species$Species_ID %in% Detection_stability$Species_ID[i] &
                                                                Daily_species$Cutoff %in% Detection_stability$Cutoff[i] & 
                                                                Daily_species$Period %in% 'Pre-typhoon'])
      Data_post<-data.frame("Detect" = Daily_species$Detect.Num[Daily_species$Site_ID %in% Detection_stability$Site_ID[i] &
                                                                Daily_species$Species_ID %in% Detection_stability$Species_ID[i] &
                                                                Daily_species$Cutoff %in% Detection_stability$Cutoff[i] & 
                                                                Daily_species$Period %in% 'Post-typhoon'])
  
      # get means and sample sizes of pre- and post-typhoon data
      # NB: pre-disturbance mean is just the same as baseline
      Detection_stability$Mean_post[i]<-Data_post$Detect %>% mean(na.rm = T)
      Detection_stability$n_pre[i]<-Data_pre$Detect[Data_pre$Detect %!in% NA] %>% length()
      Detection_stability$n_post[i]<-Data_post$Detect[Data_post$Detect %!in% NA] %>% length()

      # for cases where sample sizes >5 for both the pre- and post-typhoon period
      if(Data_pre$Detect[Data_pre$Detect %!in% NA] %>% length() > 5 & Data_post$Detect[Data_post$Detect %!in% NA] %>% length() > 5){
        
        # perform Wilcoxon signed rank test to determine whether pre- and post-typhoon data differ significantly
        Wilcox.out<-wilcox.test(x = Data_post$Detect, 
                                y = Data_pre$Detect, 
                                na.rm=TRUE, 
                                paired=FALSE, # unpaired accounts for uneven sample sizes
                                exact=FALSE, 
                                conf.int=TRUE, 
                                digits.rank = 7)

        # get extent of change values and confidence intervals
        Detection_stability$Ext_Chng[i]<-Wilcox.out$estimate
        Detection_stability$Chng_hi[i]<-Wilcox.out$conf.int %>% max(na.rm = T)
        Detection_stability$Chng_lo[i]<-Wilcox.out$conf.int %>% min(na.rm = T)
        
        # get Wilcoxon test statistic and p-value
        Detection_stability$Wilcox_W[i]<-Wilcox.out$statistic
        Detection_stability$p_val[i]<-Wilcox.out$p.value
        
      }
}

# finally mark nonsignificant [0.05 confidence level] results with a 0 (leaving significant results as a 1)
Detection_stability$p_sig[Detection_stability$p_val > 0.05]<-0

# replace NaN values with NAs
Detection_stability$Mean_post[which(is.nan(Detection_stability$Mean_post))]<-NA
 
```

With this, we now have all the data we need to test our hypotheses. Next we'll move on to the statistical analyses.

## Spatial variability

Here we'll measure spatial variability as the coefficient of variation among sites at a given time (that is, time-dependent spatial variability). Then we can check whether spatial variability is different before versus after the typhoons, and whether sites at the opposite extremes of our land use gradients differ in their spatial variability. 

First we'll make the output matrix: 

```{r}

# cut off at 0.5 detection fidelity
Daily_species2<-Daily_species[Daily_species$Cutoff %in% 0.5,]
...
Sp_Spatial_Var<-data.frame("Date"=rep(unique(Daily_species2$Date),3),
                           ...
                           "Spatial_Var"=NA,"Forest_Var"=NA,"Developed_Var"=NA)

Sp_Spatial_Var<-Sp_Spatial_Var %>% typhoon.period() # get typhoon periods

# parse_Date
Sp_Spatial_Var$Date <- as.character(Sp_Spatial_Var$Date) %>% parse_date()

```

Next, calculate spatial variability as the coefficient of variation on each date across all sites, following Donohue _et al._ (2013 _Ecol. Lett._): 
...
for (i in 1:length(unique(Sp_Spatial_Var$Date))) { # for each time point
  for (j in 1:3) { # and each species
    ...
    Sp_Spatial_Var$Spatial_Var[Sp_Spatial_Var$Date %in% unique(Sp_Spatial_Var$Date)[i] & 
...
Now we have a measure of spatial variability across all sites, but with this data we can't get at any potential differences among sites of different land use. So now we'll make the same calculations for our 2 kmeans landuse clusters from above (forest vs developed sites):
...
# replace NaN with NA
Sp_Spatial_Var[,c(3:5)] <- Sp_Spatial_Var[,c(3:5)] %>% mutate_all(~ifelse(is.nan(.),NA,.))

```

So now we have our spatial variability results for all the sites at each time point, and for the kmeans clusters subsets of landuse.

**Convert to tidy dataset**

First we need to convert the current dataframes we'll be using into a tidy dataset with rows representing observations, and columns the associated variables or metadata: 

```{r}

wide.stability_bird<-tibble::tibble(Detection_stability[,c(1:6,9,14,17)]) # convert to wide format tible
tidy.stability_bird<-wide.stability_bird %>% gather(key = Stability_dimension,value = Stability,na.rm = F,-Site_ID,-Species_ID,-Cutoff,-kmeans,-p_sig) # convert to long format (melted)

wide.spatial_bird<-tibble::tibble(Sp_Spatial_Var) # convert to wide format tibble
tidy.spatial_bird<-wide.spatial_bird %>% gather(key = Stability_dimension,value = Stability,na.rm = F,-Date,-Species,-Period) # convert to long format (melted)
tidy.spatial_bird <- tidy.spatial_bird[complete.cases(tidy.spatial_bird),]
