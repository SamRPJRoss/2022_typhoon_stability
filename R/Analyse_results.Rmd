...
# fix broken forest vs developed results
Break_results[Break_results$Species %in% 'Otus_elegans' & Break_results$Landuse %in% 'Forest' & Break_results$Period %in% 'Pre-typhoon',c(4:6)]<-Break_results[Break_results$Species %in% 'Otus_elegans' & Break_results$Landuse %in% 'Developed' & Break_results$Period %in% 'Pre-typhoon',c(4:6)]
Break_results[Break_results$Species %in% 'Otus_elegans' & Break_results$Landuse %in% 'Forest' & Break_results$Period %in% 'Post-typhoon',c(4:6)]<-Break_results[Break_results$Species %in% 'Otus_elegans' & Break_results$Landuse %in% 'Developed' & Break_results$Period %in% 'Post-typhoon',c(4:6)]
Break_results[Break_results$Species %in% 'Otus_elegans'&Break_results$Landuse %in% 'Developed' & Break_results$Period %in% 'Pre-typhoon',c(4:6)]<-NA
Break_results[Break_results$Species %in% 'Otus_elegans'&Break_results$Landuse %in% 'Developed' & Break_results$Period %in% 'Post-typhoon',c(4:6)]<-NA
...
#df$Landuse<-df$Landuse %>% as.character() %>% parse_factor(levels = c('Forest','Developed'))
#df$Period<-df$Period %>% as.character() %>% parse_factor(levels = c('Pre-typhoon','Post-typhoon'))
#df$Species<-df$Species %>% as.character() %>% parse_factor(levels = c('Corvus_macrorhynchos','Horornis_diphone','Otus_elegans'))
```
...
THE COLOURS ARE WRONG IN THE FIRST PANEL - WHY DOES THIS DATAFRAME SEEM TO THINK THE OWLS ARE ONLY IN THE DEVELOPED SITES (ERROR IS IN BIRD DATA - stored as developed instead of forest...)

Quick fix is change forest and developed in df (do it above so don't need to change it in break_results)
