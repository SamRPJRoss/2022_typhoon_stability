...
```{r}
COL <- adjustcolor(c("#29a62b","#a629a4")[df$Landuse],alpha.f = 0.2)

plot(x = (df$Date_Time),
     y = df$Stability,
     col = COL, 
     xlab = 'Date',
     ylab = 'NDSI Spatial Variability')
abline(v = df$Date_Time[df$Period %in% 'Trami'][1],lwd = 2,lty = 'dashed')
abline(v = df$Date_Time[df$Period %in% 'Post-typhoon'][1],lwd = 2,lty = 'dashed')
plot(os,term = date_for, 
     col="#29a62b", interc=FALSE, 
     const=coef(os)["LanduseForest"], 
     add=T, lwd=2)
plot(os,term = date_dev, 
     col="#a629a4", interc=FALSE,
     const=coef(os)["LanduseDeveloped"], 
     add=T, lwd=2)


df %>% ggplot(aes(x = Date_Time,
                  y = Stability,
                  col = Landuse)) +
...
predict.segmented(os) %>% plot()

predict.segmented(os)[1:3072] %>% plot()
predict.segmented(os)[3073:6144] %>% plot()

```

Find a way to get the intercepts and slopes of the broken lines and plot those as simple geom_lines or annotations or something in the nicer ggplot
