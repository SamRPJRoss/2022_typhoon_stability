...
Not using same Bayesian models for spatial variability here. Instead, using break-point models to see if spatial variability changes after typhoons, and whether values and value change differ by land use. 
...
os <- segmented(olm, 
                ...
                psi = 3000000) # where to break
AIC(os,olm) # break point model performs better

davies.test(olm)
summary(os)
#intercept(os)$Date_Time + coef(os)['Date_Time']
#slope(os)$Date_Time
#vcov.segmented(os)

# visualise breakpoint
plot.segmented(os)

# get the location of the breakpoint (+/- error)
Psi_min<-summary(os)$psi[2]-summary(os)$psi[3]
Psi_med<-summary(os)$psi[2]
Psi_max<-summary(os)$psi[2]+summary(os)$psi[3]

# get dates of breakpoint
Psi_min<-df$Date_Time[min(max(which(df2$Date_Time <= Psi_min),na.rm = T), min(which(df2$Date_Time == Psi_min),na.rm = T),na.rm = T)]
Psi_med<-df$Date_Time[min(max(which(df2$Date_Time <= Psi_med),na.rm = T), min(which(df2$Date_Time == Psi_med),na.rm = T),na.rm = T)]
Psi_max<-df$Date_Time[min(max(which(df2$Date_Time <= Psi_max),na.rm = T), min(which(df2$Date_Time == Psi_max),na.rm = T),na.rm = T)]

# compare pre- and post-break acoustic index values
Break_results<-data_frame(Period = c('Pre-break','Post-break'), Lo_95 = NA, Med = NA, Up_95 = NA)
Break_results$Lo_95[Break_results$Period %in% 'Pre-break'] <- df$Stability[df$Date_Time < Psi_min] %>% quantile(c(0.05)) %>% as.numeric()
Break_results$Med[Break_results$Period %in% 'Pre-break']  <- df$Stability[df$Date_Time < Psi_min] %>% quantile(c(0.5)) %>% as.numeric()
Break_results$Up_95[Break_results$Period %in% 'Pre-break']  <- df$Stability[df$Date_Time < Psi_min] %>% quantile(c(0.95)) %>% as.numeric()
Break_results$Lo_95[Break_results$Period %in% 'Post-break']<-df$Stability[df$Date_Time > Psi_max] %>% quantile(c(0.05)) %>% as.numeric()
Break_results$Med[Break_results$Period %in% 'Post-break']<-df$Stability[df$Date_Time > Psi_max] %>% quantile(c(0.5)) %>% as.numeric()
Break_results$Up_95[Break_results$Period %in% 'Post-break']<-df$Stability[df$Date_Time > Psi_max] %>% quantile(c(0.95)) %>% as.numeric()
Break_results$Period<-Break_results$Period %>% as.character() %>% parse_factor(levels = c('Pre-break','Post-break'))
...
plot1<-df %>% ggplot(aes(x = Date_Time,
  ...
  geom_pointrange(y = min(df$Stability), xmin = Psi_min, xmax = Psi_max,x = Psi_med) + 
  labs(x = "Time",
...
FIND OUT WHY THIS BREAK POINT IS IN A SUPER WEIRD PLACE? IS IT POSSIBLE TO FORCE A BREAK POINT?
...
os <- segmented(olm, 
                ...
                psi = list(date_for=3000000,date_dev=3000000)) # where to break

AIC(os,olm) # break point model performs better
#intercept(os)$date_for + coef(os)["LanduseForest"]
#slope(os)$date_for
#intercept(os)$date_dev + coef(os)["LanduseDeveloped"]
#slope(os)$date_dev
#vcov.segmented(os)

#davies.test(olm,seg.Z = )
summary(os)

# visualise breakpoint
#plot.segmented(os)$psi1.date_for

# get the location of the breakpoint (+/- error)

for_min<-summary(os)$psi[3]-summary(os)$psi[5] # forest
for_med<-summary(os)$psi[3]
for_max<-summary(os)$psi[3]+summary(os)$psi[5]
for_min<-df$Date_Time[min(max(which(df2$Date_Time <= for_min),na.rm = T), min(which(df2$Date_Time == for_min),na.rm = T),na.rm = T)] # get dates of breakpoint - forest
for_med<-df$Date_Time[min(max(which(df2$Date_Time <= for_med),na.rm = T), min(which(df2$Date_Time == for_med),na.rm = T),na.rm = T)]
for_max<-df$Date_Time[min(max(which(df2$Date_Time <= for_max),na.rm = T), min(which(df2$Date_Time == for_max),na.rm = T),na.rm = T)]

dev_min<-summary(os)$psi[4]-summary(os)$psi[6] # developed
dev_med<-summary(os)$psi[4]
dev_max<-summary(os)$psi[4]+summary(os)$psi[6]
dev_min<-df$Date_Time[min(max(which(df2$Date_Time <= dev_min),na.rm = T), min(which(df2$Date_Time == dev_min),na.rm = T),na.rm = T)] # get dates of breakpoint - dev
dev_med<-df$Date_Time[min(max(which(df2$Date_Time <= dev_med),na.rm = T), min(which(df2$Date_Time == dev_med),na.rm = T),na.rm = T)]
dev_max<-df$Date_Time[min(max(which(df2$Date_Time <= dev_max),na.rm = T), min(which(df2$Date_Time == dev_max),na.rm = T),na.rm = T)]

# compare pre- and post-break acoustic index values
Break_results<-data_frame(Period = c('Pre-break','Pre-break','Post-break','Post-break'), 
                          Landuse = c('Forest','Developed','Forest','Developed'), 
                          Lo_95 = NA, Med = NA, Up_95 = NA)
Break_results$Lo_95[Break_results$Period %in% 'Pre-break' & 
                      Break_results$Landuse %in% 'Forest']<-df$Stability[df$Date_Time < for_min] %>% quantile(c(0.05)) %>% as.numeric()
Break_results$Med[Break_results$Period %in% 'Pre-break' & 
                    Break_results$Landuse %in% 'Forest']  <- df$Stability[df$Date_Time < for_min] %>% quantile(c(0.5)) %>% as.numeric()
Break_results$Up_95[Break_results$Period %in% 'Pre-break' & 
                      Break_results$Landuse %in% 'Forest']  <- df$Stability[df$Date_Time < for_min] %>% quantile(c(0.95)) %>% as.numeric()
Break_results$Lo_95[Break_results$Period %in% 'Post-break' & 
                      Break_results$Landuse %in% 'Forest']<-df$Stability[df$Date_Time > for_max] %>% quantile(c(0.05)) %>% as.numeric()
Break_results$Med[Break_results$Period %in% 'Post-break' & 
                    Break_results$Landuse %in% 'Forest']<-df$Stability[df$Date_Time > for_max] %>% quantile(c(0.5)) %>% as.numeric()
Break_results$Up_95[Break_results$Period %in% 'Post-break' & 
                      Break_results$Landuse %in% 'Forest']<-df$Stability[df$Date_Time > for_max] %>% quantile(c(0.95)) %>% as.numeric()
Break_results$Lo_95[Break_results$Period %in% 'Pre-break' & 
                      Break_results$Landuse %in% 'Developed']<-df$Stability[df$Date_Time < dev_min] %>% quantile(c(0.05)) %>% as.numeric()
Break_results$Med[Break_results$Period %in% 'Pre-break' & 
                    Break_results$Landuse %in% 'Developed']  <- df$Stability[df$Date_Time < dev_min] %>% quantile(c(0.5)) %>% as.numeric()
Break_results$Up_95[Break_results$Period %in% 'Pre-break' & 
                      Break_results$Landuse %in% 'Developed']  <- df$Stability[df$Date_Time < dev_min] %>% quantile(c(0.95)) %>% as.numeric()
Break_results$Lo_95[Break_results$Period %in% 'Post-break' & 
                      Break_results$Landuse %in% 'Developed']<-df$Stability[df$Date_Time > dev_max] %>% quantile(c(0.05)) %>% as.numeric()
Break_results$Med[Break_results$Period %in% 'Post-break' & 
                    Break_results$Landuse %in% 'Developed']<-df$Stability[df$Date_Time > dev_max] %>% quantile(c(0.5)) %>% as.numeric()
Break_results$Up_95[Break_results$Period %in% 'Post-break' & 
                      Break_results$Landuse %in% 'Developed']<-df$Stability[df$Date_Time > dev_max] %>% quantile(c(0.95)) %>% as.numeric()
Break_results$Period<-Break_results$Period %>% as.character() %>% parse_factor(levels = c('Pre-break','Post-break'))
```
...
plot1<-df %>% ggplot(aes(x = Date_Time,
  ...
  geom_vline(xintercept = df$Date_Time[df$Period %in% 'Post-typhoon'][1],lty = 2) + 
  geom_line(show.legend = F) + 
  geom_pointrange(y = min(df$Stability),xmin = for_min,xmax = for_max,x = for_med,col = '#29a62b') + 
  geom_pointrange(y = min(df$Stability),xmin = dev_min,xmax = dev_max,x = dev_med) + 
  labs(x = "Time",
       y = "NDSI Spatial variability") + 
  theme_cowplot(12) + 
  ylim(c(df$Stability %>% min(),
...
df$Date_Time<-df$Date_Time %>%  julian.Date() %>% as.numeric() # convert to numeric date format for model
df$Date_Time<-df$Date_Time - min(df$Date_Time) # scale date to start at zero for model

#hist(df$Stability) # looks Gaussian
olm<-lm(Stability ~ 0 + Date_Time,data = df) # linear model
summary(olm)
#plot(olm)
...
# split out time series for each land use class separately
temp<-model.matrix(~ 0 + df$Landuse)*df$Date_Time
date_for<-temp[,1]
date_dev<-temp[,2]
rm(temp)

#hist(df$Stability) # looks Gaussian
olm<-lm(Stability ~ 0 + Landuse + date_for + date_dev,data = df) # linear model
summary(olm)
#plot(olm)
