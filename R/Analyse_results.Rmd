...
mut_mod<-mod_nonspatial_beta %>%
  spread_draws(b_TyphoonPost) %>%
  mutate(site_mean = b_TyphoonPost) # get posterior distribution data for plotting
...
NB: there's no random effect in this model because date_time is different for each data point... maybe should include date as a random effect somehow? does it matter?

Without random effect, need to find another way to display the posterior. The current mcmc_plot isn't really that nice... may need to be a bit more manual.
