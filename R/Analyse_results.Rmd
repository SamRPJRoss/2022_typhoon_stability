...
temp<-model.matrix(~ 0 + df$Landuse)*(df$Date - min(df$Date))
date_for<-temp[,1]
date_dev<-temp[,2]
rm(temp)

hist(df$Stability) # looks Gaussian
olm<-lm(Stability ~ 0 + Landuse + date_for + date_dev,data = df)
summary(olm)
plot(olm)

os <- segmented(olm, 
                seg.Z = ~ date_for + date_dev,
                psi = list(date_for=35,date_dev=35))
summary(os)

# plot break point models:
with(df, plot((Date - min(Date)), Stability, col="#29a62b"))
with(subset(df, df$Landuse=="Forest"), 
     points((Date - min(Date)), Stability,
col="#a629a4"))
...
plot.segmented(os,term = date_for,res = T,conf.level = 0.95,col="#a629a4")
plot.segmented(os,term = date_dev,res = T,conf.level = 0.95,col="#29a62b")
